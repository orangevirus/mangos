...
bool Unit::IsTriggeredAtSpellProcEvent(Unit *pVictim, SpellAuraHolder* holder, SpellEntry const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, SpellProcEventEntry const*& spellProcEvent )
{
    ...
    // If PPM handling
    if(spellProcEvent && spellProcEvent->ppmRate)
    {
        // weapon based
        if (!procSpell || procSpell->DmgClass == SPELL_DAMAGE_CLASS_RANGED || procSpell->DmgClass == SPELL_DAMAGE_CLASS_MELEE)
        {
            if(!isVictim)
            {
        ...
        // we also use this for non-wepon spells
        else if (!spellProcEvent->customChance)
            chance = GetPPMProcChance(BASE_ATTACK_TIME, spellProcEvent->ppmRate);
    }
    ...
    // Fingers of Frost HACK: save roll for re-use in Frostbite trigger
    ...
    switch(dummySpell->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                case 58914:
                {
                    // try to exclude procs from hot's (spirit bonde) and no-dmg spell casts
                    if (IsPositiveSpell(procSpell->Id) || !damage)
                        return SPELL_AURA_PROC_FAILED;
                    Unit* owner = GetOwner();
                    if( !owner || owner->GetTypeId() != TYPEID_PLAYER )
                        return SPELL_AURA_PROC_OK;
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            if ( dummySpell->SpellIconID == 3579 )
            {
                ...
                if (!(procFlag & PROC_FLAG_ON_DO_PERIODIC) || HasAura(67544) || !roll_chance_i(triggerAmount))
                    return SPELL_AURA_PROC_FAILED;
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch(trigger_spell_id)
    {
        ...
        case 56453:
        {
            ...
            // Lock and Load marker already checked in code above
