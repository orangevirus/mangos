...
void Player::Update( uint32 p_time )
{
    ...
    if (Pet* pet = GetPet())
    {
        if (!pet->IsWithinDistInMap(this, GetMap()->GetVisibilityDistance()) && (!GetCharmGuid().IsEmpty() && (pet->GetObjectGuid() != GetCharmGuid())))
...
bool Player::ActivateTaxiPathTo(std::vector<uint32> const& nodes, Creature* npc /*= NULL*/, uint32 spellid /*= 0*/)
{
    ...
    if(npc)
    {
        ...
        if(IsMounted() || !GetVehicleGuid().IsEmpty())
        {
            WorldPacket data(SMSG_ACTIVATETAXIREPLY, 4);
...
void Player::RewardSinglePlayerAtKill(Unit* pVictim)
{
    ...
    if(!PvP && !(!GetVehicleGuid().IsEmpty() && (m_movementInfo.GetVehicleFlags() & VF_GIVE_EXP)))
    {
