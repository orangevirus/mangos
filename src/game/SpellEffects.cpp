...
void Spell::EffectHeal(SpellEffectIndex /*eff_idx*/)
{
    if (unitTarget && unitTarget->isAlive() && damage >= 0)
    {
        ...
        else if (m_spellInfo->TargetAuraState == AURA_STATE_SWIFTMEND && unitTarget->HasAuraState(AURA_STATE_SWIFTMEND))
        {
            ...
            tickheal *= tickcount;
            addhealth += unitTarget->SpellHealingBonusTaken(caster, m_spellInfo, tickheal, DOT);
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 58591:                                 // Stoneclaw Totem X
                {
                    ...
                    {
                        Unit* totem = unitTarget->GetTotem(TotemSlot(i));
                        if (totem)
                ...
                case 60123:                                 // Lightwell Renew
                {
                    ...
                    int32 heal = creator->SpellBaseHealingBonusDone(GetSpellSchoolMask(spellInfo))/3 + spellInfo->EffectBasePoints[0] ;
